...
if (num_args == 0) then
    ...
    write(*,1) "    input.g"
    write(*,1) "   output.g  (Optional: omit or set to 0 to use third argument)"
...
! Check that input file is a .g file, if not stop with error
if (ifile_len < 3) then
  write(*,4) "ERROR: INPUT FILE MUST BE A .g FILE"
  stop
elseif ( (num_args >= 1) .AND. (ifile((ifile_len-1):ifile_len) /= ".g") ) then
    write(*,4) "ERROR: INPUT FILE MUST BE A .g FILE"
    stop
endif
...
if (num_args >= 2) then
    if (arg(2) == "0") then
        ofile = adjustl(trim( ifile(1:(ifile_len-2)) // "_zeroexo.g" ))
    else
...
if (ofile_len < 3) then
  write(*,4) "ERROR: OUTPUT FILE MUST BE A .g FILE"
  stop
elseif ( (num_args >= 1) .AND. (ofile((ofile_len-1):ofile_len) /= ".g") ) then
    write(*,4) "ERROR: OUTPUT FILE MUST BE A .g FILE"
    stop
endif
